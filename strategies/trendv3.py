...
STRATEGY_PARAMS = {
    ...
    # ---------------- 信号过滤与确认条件 ----------------
    'trend_ma_diff': 3,                # 判断长期均线向上的计算周期跨度
    'trend_strength_buffer': 1.02,     # 多头排列强度缓冲 (中期均线需 > 长期均线 * 1.02)
...
class InstitutionalTrendStrategy(MultiStockStrategy):
    ...
    def prepare(self, data: dict) -> dict:
        ...
        for symbol, df in self.indicators.items():
            ...
            # 填充缺失值
            df = df.ffill().bfill()
            bool_cols = ['Strong_Trend', 'MA_Cross_Up', 'MACD_Bullish', 'Pullback_Support', 'Trend_Broken']
    ...
    def on_bar(self, bar, account, symbol: str):
        ...
        if row['Strong_Trend']:
            ...
            elif state['units_held'] < cfg['max_units'] and row['Pullback_Support']:
                shares = self._calculate_shares_by_budget(account, symbol, price)
                if shares > 0:
                    state['units_held'] += 1
