...
def run_portfolio_ai_optimization(data_dict, symbols, initial_capital, n_trials=150):
    """
    🧠 投资组合级 AI 贝叶斯寻优 (Portfolio-Level Optimization)
    目标：寻找一套能在多只股票上通用的反人性参数！
    """
    logger.info(f"🧠 AI 开始【多股联合盘】智能推演，目标：寻找最高通用夏普！")
    ...
    def objective(trial):
        ...
        profit_tier1 = trial.suggest_float('profit_tier1', 0.40, 0.80, step=0.05)

        bias_entry_limit = trial.suggest_float('bias_entry_limit', 1.10, 1.25, step=0.01)
        ...
        current_cfg = {
            ...
            'ma_short': ma_short, 'ma_mid': ma_mid,
            'stop_loss_pct': stop_loss_pct, 'trailing_stop_pct': trailing_stop_pct,
            'profit_tier1': profit_tier1, 'bias_entry_limit': bias_entry_limit,
