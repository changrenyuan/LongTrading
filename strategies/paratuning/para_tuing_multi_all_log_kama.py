...
def run_portfolio_ai_optimization(data_dict, symbols, initial_capital, n_trials=500):
    ...
    def objective(trial):
        ...
        calmar = float(metrics.get('卡玛比率', -999.0))

        if calmar > trial.study.user_attrs.get("best_calmar", -999.0):
            trial.study.set_user_attr("best_calmar", calmar)
            best_metrics = metrics
            best_df_results = df_results
            print(
                f"   🚀 AI 联合盘突破! 总盘卡玛比 {calmar:.2f} | 夏普 {metrics.get('夏普比率')} | 收益 {metrics.get('累计收益率')} | 回撤 {metrics.get('最大回撤')}")
    ...
    logger.info(f"💾 【重要】已将本次寻优的 {len(df_trials)} 组全量参数及卡玛成绩保存至: {log_filename}")

    # 💡 2. 打印 Top 20 最佳参数组合排行榜
    print("\n" + "★" * 120)
    print(f"🏆 AI 寻优 Top 20 最佳参数组合排行榜 (多股联合卡玛)")
    print("★" * 120)
    for idx in range(min(20, len(df_trials))):
        ...
        print(f"Top {idx + 1:<2} | 卡玛: {sharpe:<5.2f} | {p_ma:<18} | {p_risk:<22} | {p_buy:<20} | 详情见CSV")
    print("★" * 120 + "\n")
