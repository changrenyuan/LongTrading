...
def run_portfolio_ai_optimization(data_dict, symbols, initial_capital, n_trials=100):
    """
    ...
    def objective(trial):
        ...
        # 熔断门槛
        if win_rate < 0.45 or max_dd > 0.15: score = -100.0
    ...
    with open("data/best_params_win50p.json", "w", encoding='utf-8') as f:
...
def main():
    ...
    best_params, best_metrics, best_df_res = run_portfolio_ai_optimization(
        data_dict, list(data_dict.keys()), initial_cash, n_trials=100
    )
